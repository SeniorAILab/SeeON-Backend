...
#   backend  (backend/**, backend guard scripts, or shared TS manifests): `dto:check` (BLOCK) + `tsc --noEmit` (BLOCK) + `lint` (WARN-first, decision map)
...
if [ "$be" = 1 ]; then
  gg_warn "backend changed -> dto:check + tsc --noEmit (block) + lint (warn-first, decision map)"
  pnpm --filter backend run dto:check || fail=1
  pnpm --filter backend exec tsc --noEmit || fail=1
  pnpm --filter backend run lint || gg_warn "backend lint reported issues (warn-first per decision map — not blocking)"
fi
