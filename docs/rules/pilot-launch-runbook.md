...
> **squash를 쓰면 안 된다.** #658 브랜치는 #657의 커밋 8건을 그대로 포함한다
> (야간 실측: `git rev-list origin/main..#657` 8건이 전부 #658의 조상).
...
**진행 조건:** 시설 ID가 의도한 값이고, 이벤트 URL이 가짜 도메인이 아니다.

> **이벤트 URL은 화면에 안 나온다.** 연결 설정 화면이 보여주는 것은
> 시설 ID / 시설 토큰 / 마지막 동기화 / 클라우드 전송 넷뿐이다(소스 확인).
...
`db` / `backend` / `front` 세 컨테이너가 `Up`이어야 한다. 하나라도 빠졌거나
재시작을 반복하면 5단계로 간다.

> **`Up`만으로는 배포 성공이 아니다.** 배포 스크립트의 `verify_services`가
> 보는 것은 더 구체적이다(`iwinv-deploy.sh:343-360`).
>
> - backend: 컨테이너 안에서 `/health`를 부르고 **`sha`가 방금 배포한
>   SHA와 같고 `database`가 `ok`** 여야 한다. 즉 옛 컨테이너가 살아 있어도
>   실패한다.
> - front: 컨테이너 안에서 `version.txt`를 읽어 확인한다.
>
> 배포가 성공했다면 이 검사는 이미 통과한 것이다. 여기서 `docker ps`를
> 보는 이유는 **그 뒤에 컨테이너가 죽지 않았는지** 보기 위해서다.

브라우저로 연다. **7단계 오라클을 잴 화면과 같은 곳이다** — 오라클은 이
주소에서만 성립하므로 여기서 미리 열어 두면 된다.
