...
ADRs are permanent. Never delete an ADR to “clean up” history. If a decision changes, write a successor ADR and mark the source ADR `Superseded` or `Partially Superseded`. A fully superseded ADR remains a historical source record and is excluded from active MECE validation.
...
- **Historical original**: fully superseded source record; kept for traceability and excluded from active MECE counts.
...
| [ADR-023](./common/ADR-023-ml-backend-prediction-boundary.md) | ML prediction boundary and backend product-policy ownership | Accepted | 2026-06-13 | ML returns model signals; backend owns alert policy, persistence, deduplication, rate limits, and side effects. |
...
| # | Title | Status | Date | Summary |
|---|---|---|---|---|
| [ADR-003](./ml/ADR-003-ml-serving-training-split.md) | ML serving/training lifecycle split and responsibility boundary | Superseded | 2026-06-07 | Historical source split into ADR-015, ADR-022, ADR-023, and ADR-024. |
| [ADR-004](./ml/ADR-004-relocate-video-data-to-ml-data.md) | Relocate video data from `assets/` to `ml/data/` | Superseded | 2026-06-07 | Historical source for original relocation; current active data layout is ADR-012. |
| [ADR-005](./ml/ADR-005-yolo26-pose-and-module-seam.md) | YOLO26-pose stack and two-seam module architecture | Superseded | 2026-06-08 | Historical source split into ADR-025, ADR-026, and ADR-027. |
...
| [ADR-007](./ml/ADR-007-ml-local-filesystem-layout.md) | `ml/` local filesystem layout | Superseded | 2026-06-09 | Historical source superseded by ADR-012 and ADR-015. |
...
| [ADR-012](./ml/ADR-012-ml-data-domain-first-layout.md) | Domain-first two-tier layout for `ml/data/` | Accepted; partially superseded by ADR-028 | 2026-06-10 | `ml/data/{domain}/{raw,processed,poses,annotated}` plus top-level `eval/` and `uploads/`; access boundary extracted to ADR-028. |
...
| [ADR-026](./ml/ADR-026-frame-model-seam-architecture.md) | Frame and model seam architecture | Accepted | 2026-06-13 | Extracted stream/model seam architecture from ADR-005. |
...
Counts after this reorganization:

- Existing ADRs audited: 21 (`ADR-001` through `ADR-021`).
- Existing identifiers preserved: 21/21.
- New successors created: 7 (`ADR-022` through `ADR-028`).
- Active ADRs by category: `common` 7, `backend` 1, `frontend` 0, `ml` 16.
- Historical originals excluded from active MECE validation: 4 (`ADR-003`, `ADR-004`, `ADR-005`, `ADR-007`).
...
| ADR-003 | ML serving/training split | `docs/decisions/ADR-003-ml-serving-training-split.md` | `docs/decisions/ml/ADR-003-ml-serving-training-split.md` | Accepted; partially superseded by ADR-015 | Superseded | historical-original | Lifecycle boundary; ML/backend prediction boundary; demo/product boundary; old artifact path | yes | ADR-015, ADR-022, ADR-023, ADR-024 | ml historical source; active successors in ml/common | common successors ADR-023/024 carry gate rationale | Original body preserved; active clauses represented by successors | status links all successors | category links updated | No source text deleted |
| ADR-004 | Relocate video data | `docs/decisions/ADR-004-relocate-video-data-to-ml-data.md` | `docs/decisions/ml/ADR-004-relocate-video-data-to-ml-data.md` | Accepted; partially superseded by ADR-012 | Superseded | historical-original | Original `assets/`→`ml/data/` relocation; raw/gitignore invariants inherited by ADR-012 | no new successor | ADR-012 | ml historical source | — | Original body preserved; current active layout represented by ADR-012 | status link to ADR-012 | category link updated | Historical lineage retained |
| ADR-005 | YOLO26-pose and seams | `docs/decisions/ADR-005-yolo26-pose-and-module-seam.md` | `docs/decisions/ml/ADR-005-yolo26-pose-and-module-seam.md` | Accepted; partially verified | Superseded | historical-original | Pose framework; seam architecture; output/baseline policy | yes | ADR-025, ADR-026, ADR-027 | ml historical source | — | Original body and verification preserved; active clauses represented by successors | status links all successors | category links updated | No verification evidence omitted |
...
| ADR-007 | ML local filesystem layout | `docs/decisions/ADR-007-ml-local-filesystem-layout.md` | `docs/decisions/ml/ADR-007-ml-local-filesystem-layout.md` | Accepted; partially superseded by ADR-012 and ADR-015 | Superseded | historical-original | Legacy weight/output layout | no new successor | ADR-012, ADR-015 | ml historical source | — | Original body preserved; active paths represented by ADR-012/015 | status links successors | category links updated | Historical source retained |
...
| ADR-015 | ML models single root | `docs/decisions/ADR-015-ml-models-single-root.md` | `docs/decisions/ml/ADR-015-ml-models-single-root.md` | Accepted; partially supersedes ADR-003/007 | Accepted | active | `ml/models/` root and metadata contract | no | — | ml | — | Context/Decision/Alternatives/Consequences/Evidence preserved | unchanged | category path updated | Current model-path authority |
