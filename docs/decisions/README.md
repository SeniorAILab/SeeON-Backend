...
Numbering memo: PR #104 freezes ADR-022 through ADR-028 for the MECE split. Pending serving-predict ADRs are ADR-029 and ADR-030. PR #105 split candidates are ADR-031 through ADR-034. PR #103 alert-pipeline split lands backend ADR-035 through ADR-038.
## Category ontology
...
| [ADR-035](./backend/ADR-035-backend-orchestrated-alert-api-architecture.md) | Backend-orchestrated alert API architecture | Accepted | 2026-06-13 | Backend owns alert policy/idempotency/persistence/dispatch; consumes `/predict` signal only and exposes a separate trusted `POST /api.alerts/events` ingress (PR #103 split, issue #29). |
...
| [ADR-026](./ml/ADR-026-frame-model-seam-architecture.md) | Frame and model seam architecture | Accepted | 2026-06-13 | Extracted stream/model seam architecture from retired source ADR-005. |
...
| ADR-005 | YOLO26-pose stack and two-seam module architecture | Non-MECE bundle split into framework, seam, and output/baseline authorities | ADR-025, ADR-026, ADR-027 |
...
| ADR-005 | YOLO26-pose and seams | `docs/decisions/ADR-005-yolo26-pose-and-module-seam.md` | Retired from visible corpus; recover from git history | Accepted; partially verified | Retired after supersession | retired-source | Pose framework; seam architecture; output/baseline policy | yes | ADR-025, ADR-026, ADR-027 | not visible; active successors in ml | — | Verification table and figures are now live-preserved in ADR-025, including Room 502 25% and Room 301 51.3%; active clauses and verification evidence represented by successors | successor status clauses reference retired source id | no live markdown link to removed file | Visible corpus stays MECE; no source file retained |
