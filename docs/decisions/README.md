...
| ADR-005 | YOLO26-pose and seams | `docs/decisions/ADR-005-yolo26-pose-and-module-seam.md` | Retired from visible corpus; recover from git history | Accepted; partially verified | Retired after supersession | retired-source | Pose framework; seam architecture; output/baseline policy | yes | ADR-025, ADR-026, ADR-027 | not visible; active successors in ml | — | Source body recoverable from git history; active clauses and verification evidence represented by successors | successor status clauses reference retired source id | no live markdown link to removed file | Visible corpus stays MECE; no source file retained |
